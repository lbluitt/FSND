...
"""
In Auth0, configured Callback uri to: http://localhost:8100/tabs/user-page
"""
...
def get_token_auth_header():
    '''
    Verify that token authentication header has the expected format, if so then return the token part of the header
    '''
...
def check_permissions(permission,payload):
    '''
    Verify that permission requested is included in the payload
    '''
...
def requires_auth(permission=''):
    '''
    decorator for functions that requires authentication and specific permission prior to access the proper function
    '''
